...
def get_availability(
    ...
    gp_uavailability = [
        1 if (
            gpu.memory_free >= memory_free) and (
            gpu.load < max_load or (
                include_nan and math.isnan(
                    gpu.load))) and (
                        gpu.memory_util < max_memory or (
                            include_nan and math.isnan(
                                gpu.memory_util))) and (
                                    (gpu.id not in exclude_id) and (
                                        gpu.uuid not in exclude_uuid)) else 0 for gpu in gp_us]
        ...
        if (attr_list is not None):
            ...
            for attr_group in attr_list:
                for attr_dict in attr_group:
                    ...
                    attr_precision = '.' + \
                        str(attr_dict['precision']) if (
                            'precision' in attr_dict.keys()) else ''
                    attr_suffix = str(attr_dict['suffix']) if (
                        'suffix' in attr_dict.keys()) else ''
                    attr_transform = attr_dict['transform'] if (
                        'transform' in attr_dict.keys()) else lambda x: x
                    for gpu in gp_us:
                        ...
                        if (isinstance(attr, float)):
                            ...
                            raise TypeError('Unhandled object type (' +
                                            str(type(attr)) +
                                            ') for attribute \'' +
                                            attr_dict['name'] +
                                            '\'')
                    ...
                    for gpu_idx, gpu in enumerate(gp_us):
                        ...
                        if isinstance(attr, float):
                            attr_str = (
                                '{0:' + min_width_str + attr_precision + 'f}').format(attr)
                        elif isinstance(attr, int):
                            attr_str = (
                                '{0:' + min_width_str + 'd}').format(attr)
                        elif isinstance(attr, str):
                            attr_str = (
